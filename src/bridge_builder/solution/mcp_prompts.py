"""Reference solution for stage 3: ``my_prompts.py`` with every decorator added.

Apart from this docstring, the only difference from ``my_prompts.py`` is the
``@mcp.prompt(...)`` line above each function; ``tests/test_workshop.py`` checks that.
The game serves these prompts with ``--stage 3 --solution``.
"""
